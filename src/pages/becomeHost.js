...
import Apps from '../pages/main.js';
...
export default function BecomeHost() {
  ...
  const [img1status, setImg1status] = useState("No Image Uploaded");
  const [img2status, setImg2status] = useState("No Image Uploaded");
  const [img3status, setImg3status] = useState("No Image Uploaded");
  const [img4status, setImg4status] = useState("No Image Uploaded");
    ...
    async  function uploadImageFirst(e) {
      ...
      try {
        ...
        function uploadToServer(){
          ...
          imageOne.name = (Math.random().toString(36)+'00000000000000000').slice(2, 16+2) + ".jpg";
          const uploadTask = storage.ref(`images/${userUid}/${imageOne.name}`).put(imageOne);
          uploadTask.on(
            "STATE_CHANGED",
            (snapshot) => {
              ...
              if(progress === 100){
                handleChangeImg1status("Image Has Been Uploaded")
                toast.update("1", {
      ...
      function uploadToServer(){
        ...
        uploadTask.on(
          ...
          (snapshot) => {
            ...
            if(progress === 100){
              handleChangeImg2status("Image Has Been Uploaded")
              toast.update("1", {
      ...
      function uploadToServer(){
        ...
        uploadTask.on(
          ...
          (snapshot) => {
            ...
            if(progress === 100){
              handleChangeImg3status("Image Has Been Uploaded")
              toast.update("1", {
 ...
 async  function uploadImageFourth(e) {
    ...
    function uploadToServer(){
      ...
      uploadTask.on(
        ...
        (snapshot) => {
          ...
          if(progress === 100){
            handleChangeImg4status("Image Has Been Uploaded")
            toast.update("1", {
  ...
  function handleChangeImg1status(event) {
    setImg1status(event);
  }

  function handleChangeImg2status(event) {
    setImg2status(event);
  }

  function handleChangeImg3status(event) {
    setImg3status(event);
  }

  function handleChangeImg4status(event) {
    setImg4status(event);
  }
  ...
  function handleChangeStated(event) {
    setStated(event.target.value);
  }

  function handleChangeNeigh(event) {
  ...
  const handleSubmit = (e) => {
    ...
    database.ref("properties").push({
      ...
      email: email,
      phone: phone,
      category: city + "-" + stated + "-" + neigh,
      city: city,
      stated: city + "-" + stated,
      neigh: city + "-" + stated + "-" + neigh,
      address: address,
      title: title,
      per_night: per_night,
      per_week: per_week,
  ...
  if(submit === "Submitted"){
    return (
      <>
         <Redirect to="/done-posting-listing" />
      </>
    )
  }
  ...
  return (
    ...
    <div className="create-listing">
      ...
      />
        ...
        <Card className="create-listing-card main">
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <Form.Row>
                ...
                <Form.Control type="file" onChange={uploadImageFirst} required/>
                  ...
                  <span className='label' data-js-label="file"> {img1status} </span>
                </Form.Group>
                
                <Form.Group as={Col} lg={3} md={3} sm={3} className="file-input">
                  <Form.Control type="file" onChange={uploadImageSecond} required/>
                  <span className='button'>Upload Property Image</span>
                  <span className='label' data-js-label="file">{img2status}</span>
                </Form.Group>

                <Form.Group as={Col} lg={3} md={3} sm={3} className="file-input">
                  <Form.Control type="file" onChange={uploadImageThird} required/>
                  <span className='button'>Upload Property Image</span>
                  <span className='label' data-js-label="file">{img3status}</span>
                </Form.Group>
                ...
                <Form.Group as={Col} lg={3} md={3} sm={3} className="file-input">
                  ...
                  <span className='button'>Upload Property Image</span>
                  <span className='label' data-js-label="file">{img4status}</span>
                </Form.Group>
              </Form.Row>
